...
#ifndef CERES_CAMERA_FUNCTOR_HPP
#define CERES_CAMERA_FUNCTOR_HPP
...
/**
 * @brief Ceres functor to use a Pinhole_Intrinsic_simplified (pinhole camera model K[R[t] + K1 without considering Cx,Cy) and a 3D point.
 *
 *  Data parameter blocks are the following <2,3,6,3>
 *  - 2 => dimension of the residuals,
 *  - 2=> the intrinsic data block [focal, K1],
...
struct ResidualErrorFunctor_Pinhole_Intrinsic_Simplified
{
  ResidualErrorFunctor_Pinhole_Intrinsic_Simplified(const double* const pos_2dpoint)
  {
    ...
    T* out_residuals) const
  {
    ...
    const T projected_x = focal * x_d;
    const T projected_y = focal * y_d;
...
struct ResidualErrorFunctor_Pinhole_Intrinsic_Brown_T2
{
    ...
    T* out_residuals) const
  {
    ...
    const T& focal = cam_K[OFFSET_FOCAL_LENGTH];
    const T& f_xy_ratio = cam_K[OFFSET_FOCAL_XYRATIO];
    const T& principal_point_x = cam_K[OFFSET_PRINCIPAL_POINT_X];
    const T& principal_point_y = cam_K[OFFSET_PRINCIPAL_POINT_Y];
    const T& k1 = cam_K[OFFSET_DISTO_K1];
    const T& k2 = cam_K[OFFSET_DISTO_K2];
    ...
    // Apply focal length and principal point to get the final image coordinates
    const T projected_x = principal_point_x + focal * x_d;
    const T projected_y = principal_point_y + (focal/f_xy_ratio) * y_d;
...
#endif // CERES_CAMERA_FUNCTOR_HPP
